        ...
        /* evaluate [[e->u.apply]] and return the result 39b */
        {
            ...
            case USERDEF:
                /* apply [[f.u.userdef]] and return the result 40b */
                {
                    ...
                    Valenv allval_env = mkValenv(nl, vl);
                    Namelist nl2 = f.u.userdef.locals;

                    checkargc(e, lengthNL(nl), lengthVL(vl));
                    /* set all local variables to zero */
                    while (nl2 != 0) {
                        bindval(nl2->hd, 0, allval_env);
                        nl2 = nl2->tl;
                    }

                    return eval(f.u.userdef.body, globals, functions, allval_env);
                }
            case PRIMITIVE:
